...
def annealed_ELBO(x, recon, log_p_zo, log_p_zk, log_det_sum, beta_t=1.):

    BCE = F.binary_cross_entropy(recon, x, reduction='sum') 
    log_p_x_zk = (torch.sum(log_p_zk, -1) - BCE)
...
def train_epoch(model, optimizer, tr_loader, epoch_num, steps, max_steps, device, print_progress=False):
    ...
    for batch_idx, (x, _) in enumerate(tr_loader):
        ...
        loss = annealed_ELBO(x, recon, logp_zo, logp_zk, log_det_sum, beta_t=anneling_beta_t)
...
def train_and_test(name, tr_loader, test_loader, settings, device):
    ...
    maxout_window_size, non_linearity, optim_type, flow_type, num_flow_blocks = settings
    ...
    while steps != -1:
      ...
      train_loss, steps = train_epoch(model, optimizer, tr_loader, epoch,\
